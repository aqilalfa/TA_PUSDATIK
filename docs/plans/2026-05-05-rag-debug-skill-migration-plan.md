# RAG Debug Skill Migration Implementation Plan

> **For Antigravity:** REQUIRED WORKFLOW: Use `.agent/workflows/execute-plan.md` to execute this plan in single-flow mode.

**Goal:** Migrate the Claude `rag-debug-answer` skill into the Antigravity ecosystem using the strict `writing-skills` standards.

**Architecture:** We will create a new directory and file in `.agent/skills/rag-debug-answer/SKILL.md`. We will adapt the existing content from `.claude/skills` to ensure the description only contains triggering conditions (no workflow), uses proper keywords, and follows Antigravity's structural expectations.

**Tech Stack:** Markdown, Antigravity Skill System

---

### Task 1: Create the Antigravity Skill Document

**Files:**
- Create: `.agent/skills/rag-debug-answer/SKILL.md`

**Step 1: Write the skill content**

Write the new skill file with proper frontmatter and structured content.
...
Run:
```bash
git add .agent/skills/rag-debug-answer/SKILL.md
git commit -m "docs(agent): migrate rag-debug-answer skill to Antigravity format"
```

### Task 2: Remove the old Claude skill

**Files:**
- Delete: `.claude/skills/rag-debug-answer/SKILL.md`

**Step 1: Delete file and directory**

Run:
```bash
git rm -r .claude/skills/rag-debug-answer
```
...
Run:
```bash
git commit -m "docs(agent): remove legacy claude rag-debug-answer skill"
```
