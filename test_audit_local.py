#!/usr/bin/env python
"""
Quick Test Script for Audit Logging - Local Development Version
Tests audit logging functionality with local SQLite database
"""
...
# Setup path - add backend to path BEFORE changing directory
...
DATABASE_URL = f"sqlite:///{db_dir.absolute()}/test_audit.db"
engine = create_engine(DATABASE_URL)
SessionLocal = sessionmaker(bind=engine)

print("=" * 80)
print("AUDIT LOGGING TEST SUITE — LOCAL DEVELOPMENT")
print("=" * 80)
...
# Events by IP
events_by_ip = session.query(AuditLog.ip_address).distinct().all()
...
print("=" * 80)
print(f"\nDatabase location: {db_dir.absolute()}/test_audit.db")
print("\nTo inspect the database directly:")
print(f"  sqlite3 {db_dir.absolute()}/test_audit.db")
